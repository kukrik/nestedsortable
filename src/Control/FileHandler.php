...
class FileHandler
{
    ...
    /**
     * Send HTTP headers to prevent caching and set content type to application/json
     * @return void
     */
    ...
    /**
     * Handles the file upload process, including managing chunked uploads and
    ...
    /**
     * Perform regular validation on the uploaded file.
    ...
    /**
     * Validate a file chunk during the upload process.
     *
     * @param resource|string $uploadedFile The uploaded file resource or path.
     * @param string $fileName The name of the file being uploaded.
     * @param int $error An error code that occurred during the file upload.
     * @param bool $isLastChunk Flag to indicate if the file is the last chunk.
     * @return bool True if the chunk is valid, otherwise false.
     */
    ...
    /**
     * Handles the regular file upload process.
     * @param string $uploadedFile The temporary file path of the uploaded file.
     * @param string $file The target file path where the uploaded file should be moved.
     * @return void
     */
    ...
    /**
     * Handles the chunk upload process by moving the uploaded file chunk to a temporary location,
     * merging all the chunk files once all parts are received, and performing final validations and post-processing.
     *
     * @param array $uploadedFile Information about the uploaded file chunk.
     * @param string $file The target file name for the final merged file.
     * @return void
     */
    ...
    /**
     * Deletes temporary chunk files after they have been merged into the final file.
     *
     * @param array $tempFiles A list of temporary chunk file paths to be deleted.
     * @return void
     */
    ...
    /**
     * Checks for duplicate files in the specified directory and resolves naming conflicts
     * based on the configured upload options ('overwrite' or 'increment').
     *
     * @param string $fileName The original file name to check for duplicates.
     * @return string|null New file name if incrementing, original if overwriting, or null if overwritten.
     */
    ...
    /**
     * Retrieves the error message corresponding to a given error code. If the error code
     * is not found in the predefined list of upload errors, the original error code is returned.
     *
     * @param string $error The error code for which the message is to be retrieved.
    ...
    /**
     * Processes the resizing of an image file to various dimensions and stores them in temporary folders.
     *
     * @param string $fileName The name of the image file to be resized.
     * @return void
     */
    ...
    /**
     * Creates an image resource identifier from a given path based on the image type.
     *
     * @param string $path The path to the image file.
     * @param int $type The type of the image (e.g., IMAGETYPE_JPEG, IMAGETYPE_PNG).
     * @return resource|false An image resource identifier on success, or false on errors.
     */
    ...
    /**
     * Sharpens the given image using a convolution matrix.
     *
    ...
    /**
     * Resizes an image and saves it to a new path with the specified dimensions.
     *
     * @param string $path The path to the original image.
     * @param string $newPath The path where the resized image should be saved.
     * @param int $resizeDimensions The target dimension to resize the image to.
     * @return void
     */
    ...
    /**
     * Outputs the information of the uploaded file in JSON format.
     * This includes filename, path, extension, type, error, size,
     * modified time, and dimensions of the file.
     *
     * @return void
     */
    ...
    /**
     * Handles the error scenario by creating a JSON response containing error details,
     * and optionally removes any associated temporary files or the final file.
     *
     * @param string $errorMessage The error message to be included in the JSON response.
     * @param string|null $file Optional, the file associated with the error, used for cleanup.
     * @return void
     */
    ...
    /**
     * Corrects an integer overflow issue by adjusting negative sizes to their proper values.
     *
     * @param int $size The potentially overflowed integer size.
     * @return float The adjusted size value.
     */
    ...
    /**
     * Retrieves the size of the specified file with an option to clear the stat cache.
     *
     * @param string $filePath The path to the file whose size is to be retrieved.
     * @param bool $clearStatCache Optional. Whether to clear the stat cache before retrieving the file size. Default is false.
    ...
    /**
     * Converts a configuration value with a size suffix to its equivalent value in bytes.
     *
     * @param string $val The configuration value with a potential size suffix (e.g., "2G", "512M").
    ...
    /**
     * Removes the file name from a given path, returning the directory path.
     *
     * @param string $path The full path from which the file name is to be removed.
    ...
    /**
     * Retrieves the relative path of the given absolute path by removing the root path segment.
     *
     * @param string $path The absolute path from which the relative path is to be determined.
     * @return string The relative path obtained after removing the root path segment.
     */
     ...
     * @return string|null The file extension in lowercase, or null if the path is a directory or does not have an extension.
     */
    ...
    /**
     * Returns the MIME type of the file specified by the given path.
     *
     * @param string $path The path to the file for which the MIME type is to be determined.
    ...
    /**
     * Retrieves the dimensions of an image file located at the given path.
     *
     * @param string $path The file path to the image.
     * @return string The dimensions of the image in the format "width x height", or '0 x 0' if dimensions can't be determined.
     */
     ...
     * @return string The human-readable string representing the size with appropriate units.
     */
    ...
    /**
     * Cleans the provided path by removing leading and trailing slashes and backslashes,
     * and eliminating any parent directory references to avoid path traversal.
     *
     * @param string $path The path that needs to be cleaned.
     * @return string The cleaned path with backslashes replaced by forward slashes.
     */
    ...
    /**
     * Renames a file if the new name does not already exist and the old file exists.
     *
     * @param string $old The current name of the file.
     * @param string $new The new name for the file.
     * @return bool|null Returns true if the file was successfully renamed, false if the rename operation failed,
     *                   and null if the new file name already exists or the old file does not exist.
     */
