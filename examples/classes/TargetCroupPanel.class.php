...
class TargetCroupPanel extends Q\Control\Panel
{
    ...
    public $txtFilter;
    public $dtgTargetGroup;

    public $txtTargetGroup;
    public $lstStatus;
    public $btnAddTargetGroup;
    ...
    protected $objTargetGroupIds = [];
    protected $objEventsCalendarIds;

    protected $strTemplate = 'TargetCroupPanel.tpl.php';
    ...
    public function __construct($objParentObject, $strControlId = null)
    {
        ...
        $this->checkTargetGroups();
    }

    ///////////////////////////////////////////////////////////////////////////////////////////

    /**
     * Iterates through all events calendars and collects the target group IDs.
     * If an event calendar has a target group ID, it will be added to the private
     * array property `objTargetGroupIds`.
     *
     * @return void
     */
    private function checkTargetGroups()
    {
        $objEventsCalendarArray = EventsCalendar::loadAll();

        foreach ($objEventsCalendarArray as $objEventsCalendar) {
            if ($objEventsCalendar->TargetGroupId) {
                $this->objTargetGroupIds[] = $objEventsCalendar->TargetGroupId;
            }
        }
    }
     ...
     * and editability. It also specifies a callback for row parameters and sets default sorting and pagination settings.
     *
     * @return void
     */
    protected function dtgTargetGroup_Create()
    {
     ...
     */
    protected function dtgTargetGroup_CreateColumns()
    {
     ...
     * @return void
     */
    protected function dtgTargetGroup_MakeEditable()
    {
        $this->dtgTargetGroup->addAction(new Q\Event\CellClick(0, null, Q\Event\CellClick::rowDataValue('value')), new Q\Action\AjaxControl($this, 'dtgTargetGroup_Click'));
     ...
     * @return void
     */
    protected function dtgTargetGroup_Click(ActionParams $params)
    {
     ...
     */
    public function dtgTargetGroup_GetRowParams($objRowObject, $intRowIndex)
    {
    ...
    /**
     * Initializes and configures a Select2 control for selecting items per page by an assigned user.
    ...
    /**
     * Constructs a query condition based on the current search filter input.
     *
     * @return Q\Query\QQ The constructed query condition to be used for filtering results.
     *                    Returns a condition that matches all records if the search filter is empty or null.
     *                    Otherwise, it returns a condition that searches for records where the name
     *                    contains the search value or the 'IsEnabled' status matches the search value.
     */
    public function getCondition()
    {
        ...
        if (is_null($strSearchValue) || $strSearchValue === '') {
            ...
            return Q\Query\QQ::orCondition(
                Q\Query\QQ::like(QQN::TargetGroupOfCalendar()->Name, "%" . $strSearchValue . "%"),
                Q\Query\QQ::equal(QQN::TargetGroupOfCalendar()->IsEnabled, $strSearchValue)
            );
        }
    }
     ...
     * Sets up styles, actions, and properties to ensure proper layout and functionality.
     *
     * @return void
     */
    public function createButtons()
    {
        ...
        $this->btnAddTargetGroup->addAction(new Q\Event\Click(), new Q\Action\AjaxControl($this, 'btnAddTargetGroup_Click'));
        ...
        $this->txtTargetGroup->MaxLength = TargetGroupOfCalendar::NameMaxLength;
        ...
        $this->btnDelete->addAction(new Q\Event\Click(), new Q\Action\AjaxControl($this, 'btnTargetGroupDelete_Click'));
        ...
        $this->btnCancel->addAction(new Q\Event\Click(), new Q\Action\AjaxControl($this, 'btnTargetGroupCancel_Click'));
        $this->btnCancel->setCssStyle('float', 'left');
        $this->btnCancel->Display = false;
    }

    /**
     * Initializes and configures multiple Toastr notification dialogs with predefined alert types, positions, messages, and progress bars.
     *
     * @return void
     */
    ...
    protected function createModals()
    {
        ...
        $this->dlgModal1->addAction(new Q\Event\DialogButton(), new Q\Action\AjaxControl($this, 'deleteTargetGroupItem_Click'));
     ...
     * and configuring their initial states for adding a new target group. Also manages button states based on session data.
     *
     * @param ActionParams $params The parameters for the action event.
     * @return void
     */
    protected function btnAddTargetGroup_Click(ActionParams $params)
    {
        if (!Application::verifyCsrfToken()) {
            $this->dlgModal5->showDialogBox();
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
    ...
    /**
     * Handles the click event for the save button, updating the state of the target group based on the current selection and input.
     *
    ...
    protected function btnSave_Click(ActionParams $params)
    {
        ...
        if (in_array($this->intId, $this->objTargetGroupIds)) {
            $this->lstStatus->SelectedValue = $objTargetGroup->getIsEnabled();
        ...
        } else if ($this->txtTargetGroup->Text && !in_array($this->intId, $this->objTargetGroupIds)) {
            if ($this->lstStatus->SelectedValue !== $objTargetGroup->getIsEnabled() && TargetGroupOfCalendar::titleExists(trim($this->txtTargetGroup->Text))) {
                $objTargetGroup->setName($this->txtTargetGroup->Text);
                $objTargetGroup->setIsEnabled($this->lstStatus->SelectedValue);
     ...
     * of target group IDs, showing a modal dialog and updating the display of various controls based on session data.
     *
     ...
     */
    protected function btnTargetGroupDelete_Click(ActionParams $params)
    {
        if (!Application::verifyCsrfToken()) {
            $this->dlgModal5->showDialogBox();
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
     ...
     * @return void
     */
    public function deleteTargetGroupItem_Click(ActionParams $params)
    {
        if (!Application::verifyCsrfToken()) {
            $this->dlgModal5->showDialogBox();
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
     ...
     *
     * @param ActionParams $params The parameters passed with the action event, typically includes any relevant context for the event.
     * @return void This method does not return a value.
     */
    protected function btnTargetGroupCancel_Click(ActionParams $params)
    {
        if (!Application::verifyCsrfToken()) {
            $this->dlgModal5->showDialogBox();
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
