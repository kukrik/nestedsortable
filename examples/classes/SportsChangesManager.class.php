...
class SportsChangesManager extends Q\Control\Panel
{
    ...
    /**
     * Initializes and configures the paginators for the dtgEventsChanges data grid.
     *
    ...
    /**
     * Creates and configures a UI component for selecting items per page for the assigned user.
     *
     * @return void
     */
    ...
    /**
     * Constructs and returns a query condition based on the current filter input for searching sports changes.
     *
     * @return mixed The query condition, which is either a condition to match all records when no filter is applied,
     *               or a condition to match records by ID or those containing the search value in their Title.
     */
    protected function getCondition()
    {
        ...
        if (is_null($strSearchValue) || $strSearchValue === '') {
            ...
            return Q\Query\QQ::orCondition(
                Q\Query\QQ::equal(QQN::SportsChanges()->Id, $strSearchValue),
                Q\Query\QQ::like(QQN::SportsChanges()->Title, "%" . $strSearchValue . "%")
            );
        }
    }
    ...
    /**
     * Iterates through all events in the SportsCalendar and checks for changes.
     * If an event has a change identified by getEventsChangesId(), the change ID is added
     * to the internal list of change IDs.
     *
     * @return void
     */
    ...
    /**
     * Handles the click event for the save button within the context of sports changes.
    ...
    protected function btnSave_Click(ActionParams $params)
    {
        ...
        if ($this->txtChange->Text) {
            ...
            } else if (!SportsChanges::titleExists(trim($this->txtChange->Text))) {
                $this->txtChange->Text = $objChanges->getTitle();
                $this->dlgModal4->showDialogBox();
            }
        } else {
     ...
     * a confirmation dialog is displayed and UI elements are enabled or disabled. If not, a warning dialog is shown.
     *
    ...
    /**
     * Handles the cancel action for the changes interface. Adjusts the display and enabled status of various
