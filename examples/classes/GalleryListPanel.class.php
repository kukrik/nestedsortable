...
class GalleryListPanel extends Q\Control\Panel
{
    ...
    protected $countByIsReserved;
    protected $countByAlbumsLocked;
    protected $objGroupTitleCondition;
    protected $objGroupTitleClauses;

    protected $strTemplate = 'GalleryListPanel.tpl.php';
    ...
    /**
     * Initializes and configures various buttons for the interface, including the actions and styles each button will have.
     *
     * @return void
     */
    ...
    protected function btnMove_Click(ActionParams $params)
    {
        ...
        $this->lstGalleryLocked->addAction(new Q\Event\Change(), new Q\Action\AjaxControl($this,'lstGalleryLocked_Change'));
     ...
     * @return void
     */
    protected function lstGalleryLocked_Change(ActionParams $params)
    {
        if (!Application::verifyCsrfToken()) {
            $this->dlgModal2->showDialogBox();
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
     ...
     * @return void
     */
    private function albumsTransferOperations()
    {
    ...
    protected function btnSave_Click(ActionParams $params)
    {
        ...
        if ($this->lstGroupTitle->SelectedValue === null) {
            ...
            $objGalleryList->setTitleSlug($objGallerySettings->getTitleSlug() . '/' . $verifiedName);
            ...
            $objFrontendLinks->setFrontendTitleSlug($objGallerySettings->getTitleSlug()  . '/' . $verifiedName);
     ...
     * @param string|null $relativePath An optional relative path to prepend to the gallery directory.
     * @return void
     */
    protected function makeGallery($title, $relativePath = null)
    {
        if ($relativePath) {
     ...
     * @return void
     */
    protected function dtgGallery_Create()
    {
     ...
     * @return void
     */
    protected function dtgGallery_CreateColumns()
    {
     ...
     * @return void
     */
    protected function dtgGallery_MakeEditable()
    {
        $this->dtgGallery->addAction(new Q\Event\CellClick(0, null, Q\Event\CellClick::rowDataValue('value')), new Q\Action\AjaxControl($this, 'dtgGalleryRow_Click'));
     ...
     * @return void
     */
    protected function dtgGalleryRow_Click(ActionParams $params)
    {
        if (!Application::verifyCsrfToken()) {
            $this->dlgModal2->showDialogBox();
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
     ...
     * @return array Associative array of parameters for the row, including a data-value keyed by the primary key of the row object.
     */
    public function dtgGallery_GetRowParams($objRowObject, $intRowIndex)
    {
     ...
     * It also configures the data grid to display a set number of items per page.
     *
     * @return void
     */
    ...
    /**
     * Initializes and configures the Select2 dropdown control for selecting items per page.
     *
     * The control is set with a custom theme, width, and allows single selection mode.
    ...
    /**
     * Updates the number of items per page in the gallery based on the selected value from the dropdown list.
     *
     ...
     * it adds actions for input events with a given delay and enter key events.
     * For input events, it triggers an AJAX control. For enter key events, it
    ...
    /**
     * Constructs a QQ condition based on a text input filter used for searching gallery lists.
     * If the filter input is empty or null, the query will match all entries.
     * Otherwise, it will search for matches in the GroupTitle, Title, and Author fields.
     *
     * @return \Q\Query\QQCondition The constructed QQ condition for use in database queries.
     */
    protected function getCondition()
    {
        ...
        if (is_null($strSearchValue) || $strSearchValue === '') {
            ...
            return Q\Query\QQ::orCondition(
                Q\Query\QQ::like(QQN::GalleryList()->GroupTitle, "%" . $strSearchValue . "%"),
                Q\Query\QQ::like(QQN::GalleryList()->Title, "%" . $strSearchValue . "%"),
                Q\Query\QQ::like(QQN::GalleryList()->Author, "%" . $strSearchValue . "%"),
            );
        }
    }
}