...
class SportsAreasPanel extends Q\Control\Panel
{
    ...
    protected $strTemplate = 'SportsAreasPanel.tpl.php';
    ...
    protected function dtgSportsAreas_Click(ActionParams $params)
    {
        ...
        $this->txtSportsArea->Text = $objSportsAreas->Name;
        $this->lstStatus->SelectedValue = $objSportsAreas->IsEnabled;
    ...
    public function getCondition()
    {
        ...
        if (is_null($strSearchValue) || $strSearchValue === '') {
            ...
            return Q\Query\QQ::orCondition(
                Q\Query\QQ::like(QQN::SportsAreas()->Name, "%" . $strSearchValue . "%"),
                Q\Query\QQ::equal(QQN::SportsAreas()->IsEnabled, $strSearchValue)
            );
        }
    }
    ...
    public function createButtons()
    {
        ...
        $this->btnAddSportsArea->addAction(new Q\Event\Click(), new Q\Action\AjaxControl($this, 'btnAddSportsArea_Click'));
        ...
        $this->btnSaveNew->addAction(new Q\Event\Click(), new Q\Action\AjaxControl($this, 'btnNewSave_Click'));
    ...
    /**
     * Handles the click event for the "Add Sports Area" button. This method sets various UI elements to be displayed,
     * clears the text field for sports area, focuses the text field, and handles session-related logic.
     *
     * @param ActionParams $params The parameters associated with the action event triggering this method.
     * @return void This method does not return any value.
     */
    protected function btnAddSportsArea_Click(ActionParams $params)
    {
        if (!Application::verifyCsrfToken()) {
            $this->dlgModal5->showDialogBox();
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
     ...
     * @return void
     */
    protected function btnNewSave_Click(ActionParams $params)
    {
        if (!Application::verifyCsrfToken()) {
            $this->dlgModal5->showDialogBox();
            $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
    ...
    /**
     * Handles the click event for the save button. This function updates the display and internal state
    ...
    protected function btnSave_Click(ActionParams $params)
    {
        ...
        if ($countSportsAreas > 0 || $countSelectedSportsAreas > 0) {
        ...
        } else if ($this->txtSportsArea->Text && $countSportsAreas == 0 || $countSelectedSportsAreas == 0) {
            if ($this->lstStatus->SelectedValue !== $objSportsAreas->getIsEnabled() && SportsAreas::titleExists(trim($this->txtSportsArea->Text))){
                $objSportsAreas->setName($this->txtSportsArea->Text);
                $objSportsAreas->setIsEnabled($this->lstStatus->SelectedValue);
