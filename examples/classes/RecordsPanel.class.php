<?php
    ...
    class RecordsPanel extends Q\Control\Panel
    {
        ...
        protected function createInputs(): void
        {
            ...
            $this->lstNewSportsAreas = new Q\Plugin\Select2($this);
            ...
            $this->lstNewSportsAreas->SelectionMode = ListBoxBase::SELECTION_MODE_SINGLE;
            $this->lstNewSportsAreas->addItem(t('- Select sport area -'), null, true);
            $this->lstNewSportsAreas->Enabled = false;
            $this->lstNewSportsAreas->addAction(new Change(), new AjaxControl($this, 'lstNewSportsAreas_Change'));
            ...
            $this->lstCompetitionAreas = new Q\Plugin\Select2($this);
            ...
            $this->lstCompetitionAreas->SelectionMode = ListBoxBase::SELECTION_MODE_SINGLE;
            $this->lstCompetitionAreas->addItem(t('- Select competition area -'), null, true);
            $this->lstCompetitionAreas->Enabled = false;
            $this->lstCompetitionAreas->addAction(new Change(), new AjaxControl($this, 'lstCompetitionAreas_Change'));
            ...
            $this->txtResult->CrossScripting = TextBoxBase::XSS_HTML_PURIFIER;
...
//        $this->txtResult->addAction(new Q\Event\EscapeKey(), new Q\Action\Terminate());
            ...
            $this->txtDifference->CrossScripting = TextBoxBase::XSS_HTML_PURIFIER;
...
//        $this->txtDifference->addAction(new Q\Event\EscapeKey(), new Q\Action\Terminate());
            ...
            $this->txtDetailedResult->TextMode = TextBoxBase::MULTI_LINE;
            $this->txtDetailedResult->setHtmlAttribute('autocomplete', 'off');
            $this->txtDetailedResult->CrossScripting = TextBoxBase::XSS_HTML_PURIFIER;
...
//        $this->txtDetailedResult->addAction(new Q\Event\EscapeKey(), new Q\Action\Terminate());
            ...
            $this->txtCompetitionVenue->TextMode = TextBoxBase::MULTI_LINE;
            $this->txtCompetitionVenue->setHtmlAttribute('autocomplete', 'off');
            $this->txtCompetitionVenue->CrossScripting = TextBoxBase::XSS_HTML_PURIFIER;
...
//        $this->txtCompetitionVenue->addAction(new Q\Event\EscapeKey(), new Q\Action\Terminate());
            ...
            $this->dtxCompetitionDate->AddAction(new EnterKey(), new AjaxControl($this, 'dtxCompetitionDate_EnterKey'));
            $this->dtxCompetitionDate->addAction(new EnterKey(), new Terminate());
            $this->dtxCompetitionDate->AddAction(new EscapeKey(), new AjaxControl($this, 'itemEscape_Click'));
            $this->dtxCompetitionDate->addAction(new EscapeKey(), new Terminate());
        ...
        public function createButtons(): void
        {
            ...
            $this->btnAddNewRecord->addAction(new Click(), new AjaxControl($this, 'btnAddNewRecord_Click'));
            ...
            $this->btnCompetitionDate->addAction(new Click(), new AjaxControl($this, 'btnSave_Click'));
            ...
            $this->btnCheckConfirm->addAction(new Click(), new AjaxControl($this, 'btnCheckConfirm_Click'));
        ...
        /**
         * Initializes and configures multiple Toastr notification instances with
         * different alert types, messages, and common positioning settings.
         *
         * @return void
         * @throws Caller
         */
        ...
        protected function createModals(): void
        {
            ...
            $this->dlgModal2->addAction(new Bs\Event\ModalHidden(), new AjaxControl($this, 'btnRecordCancel_Click'));
...
//                                    please unlink the record holder from both tables.</p>');
        ...
        /**
         * Handles the click action for adding a new record. It initializes and configures UI components,
         * populates dynamic elements like athlete names and sport areas, and updates the visibility
         * and interactivity of specific elements on the interface.
         *
         * @param ActionParams $params Parameters associated with the user's triggered action.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
         */
        public function btnAddNewRecord_Click(ActionParams $params): void
        {
        ...
        ");
            ...
            $this->lstAthletesNames = new Q\Plugin\Select2($this);
            ...
            $this->lstAthletesNames->SelectionMode = ListBoxBase::SELECTION_MODE_SINGLE;
            ...
            $this->lstAthletesNames->addAction(new Change(), new AjaxControl($this, 'lstAthletesNames_Change'));
        ...
        /**
         * Updates the UI elements and controls based on the selected athlete's name.
         * Enables or disables certain elements, resets input fields, and populates options
         * for available sports areas depending on the selection state of the athlete's name.
         *
         ...
         * @throws InvalidCast
         */
        protected function lstAthletesNames_Change(ActionParams $params): void
        {
        ...
        /**
         * Handles changes to the 'lstNewSportsAreas' dropdown, updating related UI elements
         * and data based on the selected value. If a valid sports area is selected, the
         * competition areas and other relevant fields are populated. If no selection is made,
         * related fields are reset and disabled.
         *
         * @param ActionParams $params Parameters associated with the action triggered by the UI event.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
         */
        protected function lstNewSportsAreas_Change(ActionParams $params): void
        {
        ...
        /**
         * Handles the change in the 'Competition Areas' dropdown and adjusts related UI components
         * based on the selected competition area and its associated details.
         *
         * @param ActionParams $params Parameters related to the triggered action.
         *
         * @return void
         ...
         * @throws InvalidCast
         */
        protected function lstCompetitionAreas_Change(ActionParams $params): void
        {
            ...
            if ($this->lstCompetitionAreas->SelectedValue !== null) {
                ...
                foreach ($objUnits as $objUnit) {

                    if ($objCompetitionAreas->UnitId == $objUnit->Id) {
                        $this->lstUnits->addItem(t('- Select Unit of measurement -'));
                        $this->lstUnits->AddItem($objUnit->Name, $objUnit->Id, true);
                        $this->lstUnits->SelectedValue = $objUnit->Id;
                    }
                }
        ...
        /**
         * Handles the Enter key event for the competition date input field.
         * Validates and processes the input when the user presses the Enter key.
         * Updates the associated record based on the provided date and lock status.
         * Displays appropriate notifications or dialog boxes based on the validation status.
         *
         ...
         * @throws InvalidCast
         */
        protected function dtxCompetitionDate_EnterKey(ActionParams $params): void
        {
...
//                }
                ...
                $objRecord = Records::load($this->intId);
        ...
        /**
         * Handles changes to the status selection.
         * Validates the input, updates the associated record's status, and displays appropriate notifications or dialog boxes.
         * Adjusts the status based on whether errors exist and the record's lock status.
         *
         * @param ActionParams $params Parameters associated with the action event.
         *
         * @return void
...
//                }
         ...
         * Executes JavaScript to hide specific UI elements, clears any user-related errors,
         * and calls a method to hide the user window.
         *
         * @param ActionParams $params Parameters associated with the action event.
         *
         * @return void
        ...
        /**
         * Handles the 'Check and Confirm' button click event.
         * Validates and processes the required inputs, updates data where necessary,
         * and generates tables for existing and new records. Provides feedback
         * via dialog boxes and inline messaging.
         *
         ...
         * @throws Exception
         */
        protected function btnCheckConfirm_Click(ActionParams $params): void
        {
            $this->checkInputs();
            ...
            if ($this->dtxCompetitionDate->Text && !$this->dtxCompetitionDate->validateFormat()) {
                    ...
                    $this->btnRecordCancel instanceof Bs\Button)
                {
                    ...
                    $this->btnConfirm->addAction(new Click(), new AjaxControl($this, 'btnConfirm_Click'));
                    ...
                    $this->btnRecordCancel->addAction(new Click(), new AjaxControl($this, 'btnRecordCancel_Click'));
                }
        ...
        /**
         * Retrieves the gender of the athlete associated with the given record.
         * Loads the athlete and their gender information based on the record's athlete ID.
         *
         * @param Records $objRecord The record containing the athlete's information.
         *
         * @return string|null The gender of the athlete, or null if the record is empty or the gender cannot be determined.
         * @throws Caller
         * @throws InvalidCast
         */
        public function getAthleteGender(Records $objRecord): ?string
        {
            if (empty($objRecord)) {
                return null;
            }
        ...
        /**
         * Retrieves the flag representation for the specified record, indicating specific conditions such as youth
         * records. Returns a flag icon in cases where the record meets the criteria, otherwise returns null.
         *
         * @param Records $objRecord The record object to evaluate flags for.
         *
         * @return string|null The flag icon as a string if the record meets the criteria, or null if it does not.
         * @throws Caller
         */
        public function getRecordsFlags(Records $objRecord): ?string
        {
            if (empty($objRecord)) {
                return null;
            }
        ...
        /**
         * Returns the rendered output of the "Is Youth" records checkbox control. If the checkbox control
         * already exists, it renders and returns its output. If the checkbox control does not exist,
         * it initializes a new checkbox control, applies necessary properties and styles, and then renders and returns
         * its output.
         *
         * @return string The rendered output of the checkbox control.
         * @throws Caller
         */
        public function IsYouthRecords(): string
        {
        ...
        /**
         * Binds data to the existing records table based on athlete's details
         * and competition-specific criteria. The method performs various
         * operations such as determining the athlete's age category, fetching
         * relevant records from the database, and configuring the display
         * logic for the records table and related elements.
         *
         * The logic implemented ensures that top records and youth records
         * are handled appropriately according to predefined rules. It also
         * includes conditional handling for enabling/disabling input controls
         * and assessing whether a new record qualifies to replace an existing
         * one. The method follows strict control order to maintain data
         * integrity and proper calculations of result differences.
         *
         * @return void This method does not return any value but modifies the
         * state of various table and control elements within the application.
         * @throws Caller
         * @throws InvalidCast
         */
        public function tblExistingRecords_Bind(): void
        {
        ...
        /**
         * Binds data to the new record table based on the selected athlete's details,
         * age category, and competition-specific information. The method calculates
         * the athlete's age category, fetches related data such as gender, competition
         * area, and other details, and then formats and binds this data to the new record
         * table for display.
         *
         * The method ensures proper transformation of input data, such as replacing
         * commas with dots in numeric fields, and organizes key pieces of information
         * like athlete name, competition details, and results to prepare a concise dataset
         * for output through the table UI.
         *
         * @return void This method does not return any value but modifies the
         * state of the new record table by populating it with formatted data and
         * updating its data source.
         * @throws Caller
         * @throws InvalidCast
         */
        public function tblNewRecord_Bind(): void
        {
            $objAthlete = Athletes::load($this->lstAthletesNames->SelectedValue);
        ...
        /**
         * Generates row-specific parameters for the existing records table.
         * This method determines visual styles or attributes such as CSS classes
         * based on the properties of the row object and its relation to records
         * like the existing record or the best record.
         *
         * @param mixed $objRowObject The row object containing record details,
         *                            used to determine the corresponding parameters.
         * @param int $intRowIndex The index of the current row, useful for row-specific logic.
         *
         * @return array An associative array containing row parameters such as CSS classes
         *               to apply for visual indication of record states (e.g., best records, interchangeable records).
         */
        ...
        /**
         * Handles the event triggered when the confirm button is clicked. This method
         * executes multiple operations including closing the modal dialog, validating
         * user inputs, creating and saving a new record, and updating associated data
         * entities. It ensures data integrity by locking records and entities, handling
         * conflicts with existing records, and managing record additions or deletions
         * based on predefined rules.
         *
         * Additionally, it updates the user interface to reflect the current state,
         * such as enabling or disabling controls and refreshing dropdown lists.
         *
         * @param ActionParams $params Contains the parameters passed during the button
         * click event, such as any contextual information necessary for processing.
         *
         * @return void This method does not return any value but modifies the state
         * of the application by performing database operations and updating UI elements.
        ...
        /**
         * Handles the cancel action for the record process. This method is triggered
         * when the cancel button is clicked and performs several UI-related
         * operations, such as hiding specific setting and action elements,
         * closing user interface windows (e.g., modals), and restoring button states.
         *
         * The method utilizes JavaScript execution to hide UI components and ensures
         * proper user interaction by resetting affected elements.
         *
         * @param ActionParams $params Parameters associated with the triggered action event.
         * These may include event-specific details needed for execution context.
         *
         * @return void This method does not return any value but modifies the application state
         * by manipulating UI elements and invoking other methods to reset interface controls.
         * @throws Caller
         */
        protected function btnRecordCancel_Click(ActionParams $params): void
        {
            // Peidab seaded ja vormi tegevuste elemendid kasutajaliidesest JavaScript'i abil.
            Application::executeJavaScript("
                $('.setting-wrapper').addClass('hidden');
        ...
        /**
         * Handles the event triggered by clicking the "Back" button.
         * Redirects the user to the statistics list page, effectively
         * allowing navigation back to the overview or summary screen.
         *
         * @param ActionParams $params Contains the parameters associated with
         * the button click event, such as context or control-specific data.
         *
         * @return void This method does not return any value but performs a
        ...
        /**
         * Handles the logic for hiding specific UI elements and triggering actions to reset
         * or update the state of the interface when the corresponding button is clicked.
         * This method applies CSS classes to certain elements to hide them and closes
         * a modal dialog box.
         *
         * The method uses JavaScript execution to manipulate front-end elements and ensures
         * that the modal dialog box associated with `dlgModal1` is closed. The commented-out
         * code indicates further UI state updates, such as enabling disabled controls, which
         * may be conditionally implemented if needed in the future.
         *
         * @param ActionParams $params The parameters passed from the action triggering the method,
         *                             typically containing context-specific data such as source
         *                             element or user interaction details.
         *
         * @return void This method does not return any value but alters the UI state by hiding
         *              certain elements and closing the modal dialog.
         * @throws Caller
         */
...
//        $this->dtgRecords->removeCssClass('disabled');
        ...
        /**
         * Validates user inputs for competition details and athlete eligibility.
         * This method performs various checks on input fields, such as ensuring required
         * fields are filled, validating the athlete's eligibility based on age limits
         * and competition rules, and cleaning specific data inputs.
         *
         * If any validation errors occur, corresponding notifications are shown, form
         * fields are dynamically marked as required, and error identifiers are added
         * to the errors array. The method also ensures proper handling of field attributes
         * and removes improper input values when necessary.
         *
         * @return void This method does not return any value but modifies the state of
         * various input fields, error tracking, and notification systems in response to
         * validation outcomes.
        ...
        /**
         * Resets and refreshes input controls in the application by updating
         * selection lists and removing required attributes from specific input fields.
         * This ensures that inputs are properly reset and ready for new data entry.
         *
         * The method performs operations that refresh lists of selectable items
         * and clears any existing mandatory constraints for certain input fields,
         * preparing the application for subsequent user actions or data processing.
         *
         * @return void This method does not return any value but alters the
         * state of input controls by refreshing lists and removing required attributes.
         */
        ...
        /**
         * Activates and populates input controls in the form using data derived from
         * the provided edit object. The method establishes proper bindings between
         * form elements and the corresponding properties of the given object.
         *
         * The method is responsible for setting the selected values for dropdown
         * menus, updating text fields, and ensuring that the form reflects
         * the state of the given data object accurately.
         *
         * @param object $objEdit The object containing data for populating the form
         * inputs. This object is expected to include accessors to retrieve athlete
         * details, competition area information, and other relevant properties.
         *
         * @return void This method does not return any value but updates multiple form
         * controls based on the data from the provided object.
         * @throws Caller
         * @throws InvalidCast
         */
        ...
        /**
         * Saves input data from the provided form elements into the specified object.
         * The method retrieves and processes related information such as selected athlete,
         * unit type, age category, and competition data, and then sets these values
         * into the appropriate fields of the given object.
         *
         * @param object $objEdit The object whose properties will be updated with the input values.
         *
         * @return void This method does not return any value but modifies the given object's properties
         * and processes related form data.
         * @throws Caller
         * @throws InvalidCast
         */
        public function saveInputs(object $objEdit): void
        {
            ...
            if ($this->chkRecordStatus->Checked) {
        ...
        /**
         * Updates the provided record with the current date, assigns the editor, saves the record,
         * updates various user interface elements, and refreshes the display.
         *
         * @param object $objRecord The record object to be updated and validated.
         *
         * @return void
         */
        protected function updateAndValidateRecord(object $objRecord): void
        {
         ...
         *
         * @param \QCubed\QDateTime $birthDate The birth date of the individual.
         * @param \QCubed\QDateTime $competitionDate The date of the competition to calculate the age against.
         *
         * @return int|null The ID of the matching age category, or null if no category matches.
         * @throws Caller
         */
        protected function calculateAgeCategory(QDateTime $birthDate, QDateTime $competitionDate): ?int
        {
        ...
        /**
         * Determines if an athlete is eligible for the youth category based on their birth date and
         * the competition date by comparing the athlete's age to the maximum permissible age
         * in the relevant youth categories.
         *
         * @param \QCubed\QDateTime $birthDate The birth date of the athlete.
         * @param \QCubed\QDateTime $competitionDate The date of the competition.
         *
         * @return bool Returns true if the athlete fits within the age limits of the youth category,
         *              otherwise returns false.
         * @throws Caller
         */
        protected function eligibleForYouthCategory(QDateTime $birthDate, QDateTime $competitionDate): bool
        {
        ...
        /**
         * Determines if the athlete's age at the time of the competition is below the youngest minimum age limit.
         *
         * @param \QCubed\QDateTime $birthDate The athlete's date of birth.
         * @param \QCubed\QDateTime $competitionDate The date of the competition.
         *
         * @return bool True if the athlete's age is below the minimum age limit, false otherwise.
         * @throws Caller
         */
        protected function youngestAgeLimitExceeded(QDateTime $birthDate, QDateTime $competitionDate): bool
        {
         ...
         */
        protected function cleanAndValidateInput(string $input): bool|string
        {
        ...
        /**
         * Finds and returns the ID of the record with the best result from the provided array of records.
         *
         * @param array $objRecords An array of record objects to evaluate. Each record object should have
         *                          a 'result' property and an 'Id' property.
         *
         * @return mixed Returns the ID of the record with the best result, or null if the input array is empty.
         */
        protected function findBestRecordId(array $objRecords): mixed
        {
        ...
        /**
         * Searches through the provided records to find an existing record ID
         * that matches specific conditions based on youth records and age group.
         *
         * @param array $objRecords An array of record objects to search through.
         * @param mixed $objAgeGroup The age group used to filter and match records.
         *
         * @return mixed Returns the ID of the matching record or null if no match is found.
         */
        protected function findExistingRecordId(array $objRecords, mixed $objAgeGroup): mixed
        {
        ...
        /**
         * Compares two records to determine if the current record is better based on the unit type.
         *
         * @param mixed $objCurrentRecord The current record value to be evaluated.
         * @param mixed $objBestRecord The previously considered best record value for comparison.
         *
         * @return bool Returns true if the current record is better than the best record, or false otherwise.
         * @throws Caller
         * @throws InvalidCast
         */
        protected function isBetterResult(mixed $objCurrentRecord, mixed $objBestRecord): bool
        {
            $objUnitType = SportsUnits::load($this->lstUnits->SelectedValue);
