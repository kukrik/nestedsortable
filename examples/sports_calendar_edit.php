<?php
    ...
    /**
     * Class SportsCalendarEditForm
     *
     * Extends the base Form class to provide a specific implementation for managing and editing a sports calendar.
     * Includes a variety of controls, such as labels, text inputs, list selectors, date pickers, modals, and buttons.
     * These controls allow for editing details like year, institutions, sports areas, titles, event locations, event
     * dates, contact information, external links, and more. The class also handles session cleanup and integrates
     * JavaScript functionalities.
     *
     * Properties:
     * - Includes modal, toastr, calendar, and input elements for managing different parts of the sports calendar's
     * data.
     * - Provides error tracking and user session handling where applicable.
     *
     * Methods:
     * - formCreate(): Initializes form elements, loads data if an object ID is present, clears relevant sessions, and
     * sets up internal logic.
     * - popupViewer(): Adds a JavaScript event listener for opening a detailed view link associated with 'view-js'
     * elements.
     * - createInputs(): Creates and configures form inputs and controls, along with setting properties for better
     * usability and interaction.
     *
     * Additional Notes:
     * - The class includes functionality to integrate with other backend services like SportsSettings, FrontendLinks,
     * and SportsTables.
     * - Contains logic to handle logged user sessions and associate users with specific sports calendar entries.
     */
    class SportsCalendarEditForm extends Form
    {
        ...
        protected Q\Plugin\Control\Label $lblInstitutions;
        protected Q\Plugin\Select2 $lstInstitutions;

        protected Q\Plugin\Control\Label $lblSportsAreas;
        protected Q\Plugin\Select2 $lstSportsAreas;
        ...
        protected Q\Plugin\Control\Label $lblInstructionLink;
        protected Bs\Button $btnInstructionLink;
        protected Q\Plugin\Control\Label $txtInstructionLink;
        protected Q\Plugin\Select2 $lstSportsContentTypes;
        protected Q\Plugin\DateTimePicker $calShowDate;
        ...
        protected Q\Plugin\DateTimePicker $calSelectedDate;
        ...
        protected Bs\Button $btnGoToSportsAreas;
        protected Bs\Button $btnGoToInstitutions;
        ...
        protected ?object $objSportsAreas = null;
        ...
        /**
         * Initializes the form by setting up the necessary parameters, loading associated data,
         * clearing relevant session variables, and creating UI components such as inputs, buttons,
         * modals, tables, and popups.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
        ...
        /**
         * Create input controls for various fields such as year, institutions, sports areas, event title, changes,
         * event place, and event date. Sets up the necessary properties, styles, and actions for each control.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
        ...
        public function createInputs(): void
        {
            ...
            $this->txtYear->Text = $this->objSportsCalendar->Year ?? null;
            ...
            $this->lstInstitutions = new Q\Plugin\Select2($this);
            ...
            $this->lstInstitutions->addItem(t('- Choose an organizing institution'), null);
            $this->lstInstitutions->SelectedValue = $this->objSportsCalendar->OrganizingInstitutionId ?? null;
            $this->lstInstitutions->addItems($this->lstInstitutions_GetItems());
            $this->lstInstitutions->addAction(new Change(), new Ajax('lstInstitutions_Change'));
            ...
            $this->lstSportsAreas->SelectedValue = $this->objSportsCalendar->SportsAreasId ?? null;
            $this->lstSportsAreas->addItems($this->lstSportsAreas_GetItems());
            $this->lstSportsAreas->addAction(new Change(), new Ajax('lstSportsAreas_Change'));
            ...
            $this->txtTitle->Text = $this->objSportsCalendar->Title ?? '';
            ...
            $this->txtEventPlace->Text = $this->objSportsCalendar->EventPlace ?? '';
            ...
            $this->calBeginningEvent->addAction(new Q\Plugin\Event\Clear(), new Ajax('calBeginningEvent_Clear'));
            ...
            $this->calEndEvent->addAction(new Q\Plugin\Event\Clear(), new Ajax('calEndEvent_Clear'));
            ...
            $this->txtWebsiteUrl->Text = $this->objSportsCalendar->WebsiteUrl ?? '';
            ...
            $this->txtFacebookUrl->Text = $this->objSportsCalendar->FacebookUrl ?? '';
            ...
            $this->txtInstagramUrl->Text = $this->objSportsCalendar->InstagramUrl ?? '';
            ...
            $this->txtOrganizers->Text = $this->objSportsCalendar->Organizers ?? '';
            ...
            $this->txtPhone->Text = $this->objSportsCalendar->Phone ?? '';
            ...
            $this->txtEmail->Text = $this->objSportsCalendar->Email ?? '';
            ...
            $this->btnInstructionLink->addAction(new Click(), new Ajax('btnInstructionLink_Click'));
            ...
            $this->txtInstructionLink->Text = t('No specific instructions or results are available...');
            ...
            $this->txtInformation->Text = $this->objSportsCalendar->Information ?? '';
            ...
            $this->txtSchedule->Text = $this->objSportsCalendar->Schedule ?? '';
        ...
        public function createButtons(): void
        {
            ...
            $this->btnGoToInstitutions->addAction(new Click(), new Ajax('btnGoToInstitutions_Click'));

            $this->btnGoToSportsAreas = new Bs\Button($this);
            $this->btnGoToSportsAreas->Tip = true;
            $this->btnGoToSportsAreas->ToolTip = t('Go to the sports areas change manager');
            ...
            $this->btnGoToSportsAreas->addAction(new Click(), new Ajax('btnGoToSportsAreas_Click'));
        ...
        protected function createModals(): void
        {
            ...
            $this->dlgModal3->addButton(t("I'll cancel"), "no-pass", false, false, null,
                ['class' => 'btn btn-default']);
            $this->dlgModal3->addAction(new DialogButton(), new Ajax('deleteDocument_Click'));
         ...
         * @throws InvalidCast
         */
        public function lstInstitutions_GetItems(): array
        {
            $a = array();
        ...
        /**
         * Clears the text in the txtYear input field if no text is present and sets a required HTML attribute.
         *
         * @param ActionParams $params Parameters passed to the action handler.
         *
         * @return void
         */
        ...
        /**
         * Handles changes to the txtYear input field, updates the sports calendar object,
         * notifies the user, and refreshes the UI based on the input provided.
         *
         * @param ActionParams $params Parameters passed to the action handler.
         *
         * @return void
        ...
        {
            $objSportsFiles = SportsTables::loadArrayBySportsCalendarGroupId($this->intId);

            if ($this->txtYear->Text) {
                $this->objSportsCalendar->setYear($this->txtYear->Text);
                $this->objSportsCalendar->save();
                $this->objSportsCalendar->saveSportsEvent($this->objSportsCalendar->getYear(), $this->objSportsCalendar->getTitle(), $this->objSportsSettings->getTitleSlug());

                if ($objSportsFiles) {
                    foreach ($objSportsFiles as $objSportsFile) {
                        $objSportsFile = SportsTables::loadById($objSportsFile->getId());
                        $objSportsFile->setYear($this->txtYear->Text);
                        $objSportsFile->save();
                    }
                }
        ...
        /**
         * Handles the change event for the lstInstitutions dropdown. Updates the organizing institution ID
         * in the Sports Calendar if the selected value differs from the currently set value. Triggers UI
         * updates and notifications upon successful modification.
         *
         * @return void
         * @throws Caller
         */
        protected function lstInstitutions_Change(): void
        {
            if ($this->lstInstitutions->SelectedValue !== null) {
                $this->objSportsCalendar->setOrganizingInstitutionId($this->lstInstitutions->SelectedValue);
            } else {
                $this->objSportsCalendar->setOrganizingInstitutionId(null);
            }
            ...
            OrganizingInstitution::updateAllIsLockStates();
            $this->renderActionsWithId();
            $this->refreshDisplay();
        }
        ...
        protected function lstChanges_Change(ActionParams $params): void
        {
            if ($this->lstChanges->SelectedValue !== null) {
                ...
                $this->objSportsCalendar->setEventsChangesId(null);
            }
            ...
            SportsChanges::updateAllChangeLockStates();
            $this->renderActionsWithId();
            $this->refreshDisplay();
        }

        /**
         * Clears the text in the calBeginningEvent input field if no text is present and sets a required HTML
         * attribute.
         *
         * @param ActionParams $params Parameters passed to the action handler.
         *
         * @return void
         */
        protected function calBeginningEvent_Clear(ActionParams $params): void
        {
            if (!$this->calBeginningEvent->Text) {
                $this->calBeginningEvent->Text = '';
        ...
        /**
         * Clears the text in the calEndEvent input field if no text is present.
         *
         * @param ActionParams $params Parameters passed to the action handler.
         *
         * @return void
         */
        protected function calEndEvent_Clear(ActionParams $params): void
        {
            if (!$this->calEndEvent->Text) {
                $this->calEndEvent->Text = '';
            }
        }
        ...
        protected function moveItem_Click(ActionParams $params): void
        {
            ...
            $this->objSportsCalendar->updateSportsEvent($this->txtYear->Text, $this->txtTitle->Text, $objGroupTitle->getTitleSlug());
            $this->objSportsCalendar->save();
            ...
            foreach ($objSportsFiles as $objSportsFile) {
                $objSportsFile = SportsTables::loadById($objSportsFile->getId());
                $objSportsFile->setMenuContentGroupId($objGroupTitle->getMenuContentId());
                $objSportsFile->save();
            }
            ...
            $url = (isset($_SERVER['HTTPS']) ? "https" : "http") . '://' . $_SERVER['HTTP_HOST'] . QCUBED_URL_PREFIX .
                $this->objSportsCalendar->getTitleSlug();
            $this->txtTitleSlug->Text = Html::renderLink($url, $url, ["target" => "_blank"]);
            $this->txtTitleSlug->HtmlEntities = false;
            $this->txtTitleSlug->setCssStyle('font-weight', 400);
        ...
        protected function imageDelete_Push(ActionParams $params): void
        {
            ...
            $this->objSportsCalendar->setPictureDescription('');
            $this->objSportsCalendar->setAuthorSource('');
            $this->objSportsCalendar->setAssignedEditorsNameById($this->intLoggedUserId);
            $this->objSportsCalendar->save();
         ...
         * @throws Throwable
         */
        protected function btnInstructionLink_Click(ActionParams $params): void
        {
            $_SESSION["redirect-data"] = 'sports_calendar_edit.php?id=' . $this->intId . '&group=' . $this->intGroup;
        ...
        protected function btnDownloadSave_Click(ActionParams $params): void
        {
            ...
            if (count($errors) === 1) {
                ...
                if (!empty($_SESSION["data_id"]) || !empty($_SESSION["data_name"]) || !empty($_SESSION["data_path"])) {
                    ...
                    SportsContentTypes::updateAllIsLockStates();
        ...
        protected function btnSelectedSave_Click(ActionParams $params): void
        {
            ...
            if (!$this->calSelectedDate->Text) {
                $this->calSelectedDate->setHtmlAttribute('required', 'required');
                $errors[] = 'calSelectedDate';
            }
        ...
        /**
         * Handles the click event to save a button. Validates user input fields, provides error
         ...
         * displays notifications for field errors, performs partial saves when applicable, and redirects if saving
         * succeeds without errors.
         *
         * @param ActionParams $params The parameters passed with this action, typically containing event-specific
         *     data.
         *
         * @return void This function does not return a value. Instead, it handles validation, updates, and redirects
         *     directly.
         * @throws Caller
         * @throws Throwable
         */
         ...
         * Also updates and saves the corresponding frontend links with data from the sports calendar and frontend
         * options. Updates the displayed title slug link based on the sports calendar title.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
         ...
         *     data.
         *
         * @return void This function does not return a value. It updates form inputs and displays notifications as
         *     needed.
         * @throws Caller
         */
        ...
        protected  function lstStatus_Change(): void
        {
            ...
            $previousStatus = $this->objSportsCalendar->getStatus();
            ...
            if (count($this->errors)) {
                ...
                if ((int)$selectedStatus === 1) {
                    ...
                    $this->objSportsCalendar->setStatus($previousStatus);
                } else {
                    // If Hidden or Draft, we allow changing the state
                    $this->objSportsCalendar->setStatus($selectedStatus);
                }
                ...
                } else if ($this->objSportsCalendar->getStatus() === 2) {
                    $this->dlgModal4->showDialogBox();
                } else if ($this->objSportsCalendar->getStatus() === 3) {
            ...
            if ($objSportsFiles) {
                foreach ($objSportsFiles as $objSportsFile) {
                    $objSportsFile = SportsTables::loadById($objSportsFile->getId());
                    if ($this->objSportsCalendar->getStatus() === 1) {
                        $objSportsFile->setFrontendVisible(1);
                    } else {
                        $objSportsFile->setFrontendVisible(0);
                    }
                    $objSportsFile->save();
                }
            }
         ...
         * and updates the sports calendar accordingly, including setting the postulate date and assigned editors.
         *
        ...
        public function renderActionsWithId(): void
        {
            if (!empty($this->intId)) {
                if ($this->txtYear->Text !== $this->objSportsCalendar->getYear() ||
                    ...
                    $this->txtInstagramUrl->Text !== $this->objSportsCalendar->getInstagramUrl() ||
                    $this->lstInstagramTargetType->SelectedValue !== $this->objSportsCalendar->getInstagramTargetType() ||
                    $this->txtInformation->Text !== $this->objSportsCalendar->getInformation() ||
                    $this->txtSchedule->Text !== $this->objSportsCalendar->getSchedule() ||
                    $this->txtPictureDescription->Text !== $this->objSportsCalendar->getPictureDescription() ||
                    $this->txtAuthorSource->Text !== $this->objSportsCalendar->getAuthorSource() ||
        ...
        /**
         * Validates individual form fields and updates the `errors` array with field identifiers for any invalid
         * input.
         * Adds or removes CSS classes and HTML attributes to indicate validation status for each field.
         *
         * @return void This method does not return a value. It performs validation checks on form fields and updates
         ...
         * @throws Throwable
         */
        public function btnGoToInstitutions_Click(ActionParams $params): void
        {
         ...
         * @throws Throwable
         */
        public function btnGoToSportsAreas_Click(ActionParams $params): void
        {
    ...
    SportsCalendarEditForm::run('SportsCalendarEditForm');