...
class SampleForm extends Form
{
    ...
    protected $objGalleryGroupTitleCondition;
    protected $objGalleryGroupTitleClauses;
    ...
    protected $lblInserter;
    protected $txtInserter;
    ...
    protected $lblPhotoAuthor;
    protected $txtPhotoAuthor;

    protected $lblPhotoDescription;
    protected $txtPhotoDescription;

    ////////////////////////////////

    protected $txtFileName;
    protected $txtFileAuthor;
    protected $txtFileDescription;
    protected $lstIsEnabled;
    protected $btnPhotoSave;
    protected $btnPhotoCancel;
    ...
    protected $btnAlbumSave;
    protected $btnAlbumDelete;
    protected $btnAlbumCancel;
    ...
    protected $oldTitle;
    protected $oldTitleSlug;

    protected $blnCheckGallery;
    protected $intDataNumbers;
    protected $objUser;
    protected $intLoggedUserId;
    protected $intChangeFilesId = null;
    protected $intDeleteId = null;
    protected $intAlbum = null;
    protected $objAlbum = null;
    ...
    protected function formCreate()
    {
         ...
         *
         * Must to save something here $this->objGalleryList->setUserId(logged user session);
    ...
    protected function createTable()
    {
        ...
        $this->dtgAlbumList = new Q\Plugin\Control\VauuTable($this);
        ...
        $this->dtgAlbumList->Paginator = new Bs\Paginator($this);
        $this->dtgAlbumList->Paginator->LabelForPrevious = t('Previous');
        $this->dtgAlbumList->Paginator->LabelForNext = t('Next');

        $this->dtgAlbumList->ItemsPerPage = $this->objUser->ItemsPerPageByAssignedUserObject->pushItemsPerPageNum();
        ...
        $this->lstItemsPerPage->SelectedValue = $this->objUser->ItemsPerPageByAssignedUser;
        $this->lstItemsPerPage->addItems($this->lstItemsPerPageByAssignedUserObject_GetItems());
        $this->lstItemsPerPage->AddAction(new Q\Event\Change(), new Q\Action\Ajax('lstItemsPerPage_Change'));
    }

    /**
     * Retrieves a list of items per page associated with a specific user object.
     * It applies a query condition to fetch relevant items and returns them as a list.
     * Each item in the list includes its string representation and identifier.
     *
     * @return ListItem[] An array of ListItem objects representing the items per page associated with the user object.
     */
    ...
    /**
     * Handles the change in items per page for a data grid.
     * Updates the data grid to display the selected number of items per page and refreshes its content.
     *
     * @param ActionParams $params Action parameters that trigger this change.
     * @return void
     */
    public function lstItemsPerPage_Change(ActionParams $params)
    {
        $this->dtgAlbumList->ItemsPerPage = $this->lstItemsPerPage->SelectedName;
     ...
     * @return void
     */
    public function dtgAlbumList_Bind()
    {
        $this->dtgAlbumList->TotalItemCount = Album::countByFolderId($this->intFolder);
    ...
    protected function createInputs()
    {
        ...
        $this->txtTitle->AddAction(new Q\Event\EnterKey(), new Q\Action\Ajax('btnAlbumSave_Click'));
        ...
        $this->txtPhotoAuthor->Text = $this->objGalleryList->PhotoAuthor;
        $this->txtPhotoAuthor->CrossScripting = Bs\TextBox::XSS_HTML_PURIFIER;
        $this->txtPhotoAuthor->AddAction(new Q\Event\EnterKey(), new Q\Action\Ajax('btnAlbumSave_Click'));
        $this->txtPhotoAuthor->addAction(new Q\Event\EnterKey(), new Q\Action\Terminate());
        $this->txtPhotoAuthor->AddAction(new Q\Event\EscapeKey(), new Q\Action\Ajax('itemEscape_Click'));
        $this->txtPhotoAuthor->addAction(new Q\Event\EscapeKey(), new Q\Action\Terminate());
        ...
        $this->txtPhotoDescription->Text = $this->objGalleryList->PhotoDescription;
        $this->txtPhotoDescription->TextMode = Q\Control\TextBoxBase::MULTI_LINE;
        $this->txtPhotoDescription->CrossScripting = Bs\TextBox::XSS_HTML_PURIFIER;
        $this->txtPhotoDescription->AddAction(new Q\Event\EnterKey(), new Q\Action\Ajax('btnAlbumSave_Click'));
        $this->txtPhotoDescription->addAction(new Q\Event\EnterKey(), new Q\Action\Terminate());
        $this->txtPhotoDescription->AddAction(new Q\Event\EscapeKey(), new Q\Action\Ajax('itemEscape_Click'));
        $this->txtPhotoDescription->addAction(new Q\Event\EscapeKey(), new Q\Action\Terminate());
     ...
     */
    public function View_render(Album $objAlbum)
    {
     ...
     */
    public function Name_render(Album $objAlbum)
    {
     ...
     * @return mixed Returns the rendered output of the "Is Enabled" list if the album ID matches the given condition,
     *               or the status object of the album otherwise.
     */
    public function IsEnabled_render(Album $objAlbum)
    {
        if ($objAlbum->Id == $this->intChangeFilesId) {
     ...
     * @return string The rendered description or the input field for editing if in edit mode.
     */
    public function PhotoDescription_render(Album $objAlbum)
    {
        if ($objAlbum->Id == $this->intChangeFilesId) {
            return $this->txtFileDescription->render(false);
        } else {
            return $objAlbum->PhotoDescription;
        }
    }
     ...
     * @return string The rendered photo author if the album ID matches, otherwise the album's photo author.
     */
    public function PhotoAuthor_render(Album $objAlbum)
    {
        if ($objAlbum->Id == $this->intChangeFilesId) {
            return $this->txtFileAuthor->render(false);
        } else {
            return $objAlbum->PhotoAuthor;
        }
    }
     ...
     *
     * This method defines multiple buttons used for file uploads, album management, and navigation within the gallery settings.
     * It assigns distinct styles, text, actions, and behaviors to each button to ensure proper functionality and user interactivity.
     *
     * @return void
     */
    public function createButtons()
    {
        ...
        $this->btnAddFiles->addAction(new Q\Event\Click(), new Q\Action\Ajax('uploadStart_Click'));
        ...
        $this->btnPhotoSave->addAction(new Q\Event\Click(), new Q\Action\Ajax('btnPhotoSave_Click'));
    ...
    {
        // https://accreditly.io/articles/handle-a-chunked-uploader-server-side-in-php
        // SUPER!!! // https://accreditly.io/articles/create-a-chunked-uploader-in-js-and-php-to-handle-large-files
    ...
    protected function createToastr()
    {
        ...
        $this->dlgToastr8->Message = t('<strong>Well done!</strong> The album and files have been successfully deleted.');
        ...
        $this->dlgToastr9->Message = t('<strong>Sorry!</strong> Failed to delete album and files.');
        ...
        $this->dlgToastr10->Message = t('<strong>Well done!</strong> The album and files have been successfully updated.');
        ...
        $this->dlgToastr11->Message = t('<strong>Sorry!</strong> Failed to update album and files.');
        ...
        $this->dlgToastr12->Message = t('<strong>Well done!</strong> The transfer of albums to the new group was successful.');
        ...
        $this->dlgToastr13->Message = t('<strong>Sorry</strong>, the transfer of albums to the new group failed.');
        $this->dlgToastr13->ProgressBar = true;

        $this->dlgToastr14 = new Q\Plugin\Toastr($this);
        $this->dlgToastr14->AlertType = Q\Plugin\Toastr::TYPE_INFO;
        $this->dlgToastr14->PositionClass = Q\Plugin\Toastr::POSITION_TOP_CENTER;
        $this->dlgToastr14->Message = t('<strong>Well done!</strong> Updates to some records for this post were discarded, and the record has been restored!');
    ...
    protected function createModals()
    {
        ...
        $this->dlgModal3->addAction(new \QCubed\Event\DialogButton(), new \QCubed\Action\Ajax('photoDeleteItem_Click'));
        ...
        $this->dlgModal6->addAction(new \QCubed\Event\DialogButton(), new \QCubed\Action\Ajax('deleteAlbum_Click'));
        ...
        $this->dlgModal11->Text = t('<p style="line-height: 25px; margin-bottom: 2px;">Are you sure you want to hide this album or edit it again?</p>
                                <p style="line-height: 25px; margin-bottom: -3px;">You can make this album public again later!</p>');
        $this->dlgModal11->Title = t('Question');
        $this->dlgModal11->HeaderClasses = 'btn-danger';
        $this->dlgModal11->addButton(t("I accept"), "pass", false, false, null,
            ['class' => 'btn btn-orange']);
        $this->dlgModal11->addCloseButton(t("I'll cancel"));
        $this->dlgModal11->addAction(new Q\Event\DialogButton(), new Q\Action\Ajax('statusItem_Click'));
        $this->dlgModal11->addAction(new Bs\Event\ModalHidden(), new Q\Action\Ajax('hideItem_Click'));

        $this->dlgModal12 = new Bs\Modal($this);
        $this->dlgModal12->Title = t("Success");
        $this->dlgModal12->HeaderClasses = 'btn-success';
        $this->dlgModal12->Text = t('<p style="line-height: 25px; margin-bottom: 2px;">This album is now hidden!</p>');
        ...
        $this->dlgModal13->Text = t('<p style="line-height: 25px; margin-bottom: 2px;">This album has now been made public!</p>');
        $this->dlgModal13->addButton(t("OK"), 'ok', false, false, null,
            ['data-dismiss'=>'modal', 'class' => 'btn btn-orange']);

        $this->dlgModal14 = new Bs\Modal($this);
        $this->dlgModal14->Title = t("Success");
        $this->dlgModal14->HeaderClasses = 'btn-success';
        $this->dlgModal14->Text = t('<p style="line-height: 25px; margin-bottom: 2px;">This album is now a draft!</p>');
        $this->dlgModal14->addButton(t("OK"), 'ok', false, false, null,
            ['data-dismiss'=>'modal', 'class' => 'btn btn-orange']);
    }
    ...
    /**
     * Handles the 'Done' button click event. Updates the settings and gallery list, locks the albums if not already locked,
     * updates the displayed post update date and list of users as editors, refreshes the display, and resets session variables.
     *
    ...
    {
        Application::executeJavaScript("
            $('.all-start').removeAttr('disabled');
            $('.all-cancel').removeAttr('disabled'); 
        ");
     ...
     */
    public function lstGalleryGroupTitle_GetItems()
    {
        $a = array();
    ...
    protected function moveItem_Click(ActionParams $params)
    {
        ...
        // Since we are using album transfer from one group to another and we need to refresh the page using Application::redirect(),
        // we can't report the album transfer success, so let these Toasts remain as they are...
    ...
    /**
     * Handles the click event on the status item, updating the gallery list's status,
     * associated metadata, and refreshing the display accordingly.
     *
    ...
    /**
     * Locks input fields and updates the gallery list status depending on the selected value.
     * This method also updates post-update date, assigns editors, and refreshes the display.
     *
     * @return void
     */
    protected function lockInputFields()
    {
        if ($this->lstStatus->SelectedValue === 1) {
            if (Album::countByFolderId($this->intFolder) == 0) {
                ...
                $this->objGalleryList->setStatus(1);
    ...
    /**
     * Handles the click event to hide an item and updates the selected value of the status list.
     *
     * @param ActionParams $params The parameters associated with the action event.
     * @return void
     */
     ...
     * @return void
     */
    public function btnAlbumSave_Click(ActionParams $params)
    {
    ...
    protected function updateAlbum()
    {
        ...
        $objGallerySettings = GallerySettings::load($this->objSettings->getId());
        ...
        if (trim($this->txtTitle->Text) !== $this->objGalleryList->getTitle()) {
            ...
            if (!file_exists($this->strRootPath . $this->objGalleryList->getPath())) {
                $this->dlgToastr11->notify();
            } else {
                $this->dlgToastr10->notify();

                unset($this->oldTitle);
                unset($this->oldTitleSlug);
            }

        } else if ($this->txtPhotoDescription->Text !== $this->objGalleryList->getPhotoDescription() ||
            $this->txtPhotoAuthor->Text !== $this->objGalleryList->getPhotoAuthor()) {

            $this->objGalleryList->setPhotoDescription($this->txtPhotoDescription->Text);
            $this->objGalleryList->setPhotoAuthor($this->txtPhotoAuthor->Text);
            $this->objGalleryList->save();
        ...
        if ($this->txtTitle->Text) {
            $url = (isset($_SERVER['HTTPS']) ? "https" : "http") . '://' . $_SERVER['HTTP_HOST'] . QCUBED_URL_PREFIX .
                $this->objGalleryList->getTitleSlug();
            $this->txtTitleSlug->Text = Q\Html::renderLink($url, $url, ["class" => "view-link", "target" => "_blank"]);
            $this->txtTitleSlug->HtmlEntities = false;
            $this->txtTitleSlug->setCssStyle('font-weight', 400);
        } else {
            $this->txtTitleSlug->Text = t('Uncompleted link...');
    ...
    protected function itemEscape_Click(ActionParams $params)
    {
        ...
        $this->txtTitle->Text = $this->objGalleryList->getTitle();
        $this->txtPhotoDescription->Text = $this->objGalleryList->getPhotoDescription();
        $this->txtPhotoAuthor->Text = $this->objGalleryList->getPhotoAuthor();
    }
     ...
     * @return void
     */
    public function btnAlbumDelete_Click(ActionParams $params)
    {
     ...
     * @return void
     */
    protected function deleteAlbum_Click(ActionParams $params)
    {
     ...
     * @return void
     */
    public function btnAlbumCancel_Click(ActionParams $params)
    {
    ...
    /**
     * Renders the appropriate set of buttons (Edit/Change and Delete) for a specific album.
     * If the album is currently being edited, renders Save and Cancel buttons instead.
     *
     * @param Album $objAlbum The album object for which buttons are being rendered.
     * @return string The rendered HTML string of the buttons.
     */
    public function Change_render(Album $objAlbum)
    {
        if ($objAlbum->Id == $this->intChangeFilesId) {
            ...
            if (!$btnChange) {
                ...
                $btnChange->Text = t('Muuda');
                $btnChange->ActionParameter = $objAlbum->Id;
                $btnChange->CssClass = 'btn btn-orange';
                $btnChange->addAction(new Q\Event\Click(), new Q\Action\Ajax('btnChange_Click'));
            ...
            if (!$btnPhotoDelete) {
                ...
                $btnPhotoDelete->Text = 'Kustuta';
                $btnPhotoDelete->ActionParameter = $objAlbum->Id;
                $btnPhotoDelete->setCssStyle('margin-top', '7px');
                $btnPhotoDelete->addAction(new Q\Event\Click(), new Q\Action\Ajax('btnPhotoDelete_Click'));
                $btnPhotoDelete->CausesValidation = false;
            }

            if ($this->intChangeFilesId) {
                $btnChange->Enabled = false;
                $btnPhotoDelete->Enabled = false;
            } else {
                $btnChange->Enabled = true;
                $btnPhotoDelete->Enabled = true;
            }
    ...
    protected function btnChange_Click(ActionParams $params)
    {
        ...
        $this->txtFileDescription->Text = $objFile->getPhotoDescription();
        $this->txtFileAuthor->Text = $objFile->getPhotoAuthor();
     ...
     * @return void
     */
    protected function btnPhotoDelete_Click(ActionParams $params)
    {
     ...
     * @return void
     */
    protected function photoDeleteItem_Click(ActionParams $params)
    {
        ...
        if (Album::countByFolderId($this->intFolder) === 0) {
            ...
            $objList->setPhotoAuthor('');
            $objList->setPhotoDescription('');
            $objList->setStatus(2);
            $objList->save();
     ...
     * @return void
     */
    protected function btnPhotoSave_Click(ActionParams $params)
    {
        ...
        } else if ((strlen($this->txtFileName->Text) == strlen($this->getFileName($this->intAlbum->getName()))) &&
            (strlen($this->txtFileAuthor->Text) !== strlen($this->intAlbum->getPhotoAuthor()) ||
                strlen($this->txtFileDescription->Text) !== strlen($this->intAlbum->getPhotoDescription()) ||
     ...
     * @return void
     */
    protected function updateFile($intAlbum)
    {
        $parts = pathinfo($this->strRootPath . $this->intAlbum->getPath());
        $files = glob($parts['dirname'] . '/*', GLOB_NOSORT);
        ...
        if (is_file($this->strRootPath . $this->intAlbum->getPath())) {
            ...
            $objAlbum->setPhotoDescription($this->txtFileDescription->Text);
            $objAlbum->setPhotoAuthor($this->txtFileAuthor->Text);
     ...
     * @return void
     */
    protected function btnPhotoCancel_Click(ActionParams $params)
    {
        $this->intChangeFilesId = null;
        $this->dtgAlbumList->refresh();
    }
    ...
    public function renderActionsWithId()
    {
        if (strlen($this->intId)) {
            if ($this->txtTitle !== $this->objGalleryList->getTitle() ||
                ...
                $this->txtFileAuthor->Text !== $this->objAlbum->getPhotoAuthor() ||
                $this->txtFileDescription->Text !== $this->objAlbum->getPhotoDescription() ||
     ...
     * @return void
     */
    public function updateGalleryFolder($destination, $currentTitleSlug, $newFolderName)
    {
    ...
    protected function fullCopy($src, $dst)
    {
        ...
        if (is_dir($src)) {
            ...
            if (file_exists($dirname . '/' . basename($name) . '.' . $ext)) {
                ...
                $dst = $dirname . '/' . $name . '-' . $inc . '.' . $ext; // Kasutame unikaalset nime
            }
     ...
     */
    protected function generateUniqueFolderName($baseFolderName, $path)
    {
        ...
        // Add index until unique name is found
     ...
     * @return int|null The ID of the matching folder if found, 1 if the path is empty, or null if no match is found.
     */
    private function getIdFromParent($path)
    {
        $objFolders = Folders::loadAll();
    ...
    /**
     * Determines the MIME type of a given file path using available PHP functions.
     *
     ...
     * @return int|null The width of the image in pixels, or null if the file is not a valid image.
     */
