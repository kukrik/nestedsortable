<?php
    ...
    /**
     * Class OrganizingInstitutionTable
     *
     * Extends the VauuTable class to represent a data table for managing
     * and displaying information about organizing institutions. This class
     * provides functionality to define columns, bind data, and manage additional
     * conditions or clauses for querying data.
     *
     * Properties:
     * - colId: The column that represents the ID of an organizing institution.
     * - colTitle: The column that represents the name of an organizing institution.
     * - colStatus: The column that represents the activation status of an organizing institution.
     * - colPostDate: The column that represents the creation date of an organizing institution.
     * - colPostUpdateDate: The column that represents the modification date of an organizing institution.
     *
     * Methods:
     * - __construct: Initializes the table, sets a data binder, and watches specified data.
     * - createColumns: Defines the columns to display in the data table.
     * - bindData: Binds data to the table using specified conditions and clauses.
     * - getCondition: Aggregates conditions for querying data, combining additional conditions with the instance's conditions.
     * - getClauses: Aggregates query clauses, combining additional clauses with the instance's clauses.
     * - __get: Retrieves the value of a property, handling special cases for 'Condition' and 'Clauses'.
     * - __set: Sets the value of a property, handling special cases for 'Condition' and 'Clauses'.
     */
    class OrganizingInstitutionTable extends VauuTable
    {
         ...
         *
         * @param mixed $objParent The parent object that this control is a part of.
         * @param string|null $strControlId Optional control identifier. If null, a unique ID will be generated.
         *
         * @return void
         * @throws Caller
         */
        public function __construct(mixed $objParent, ?string $strControlId = null)
        {
            ...
            $this->watch(QQN::OrganizingInstitution());
        }

        /**
         * Initializes and configures columns for the organizing institution table.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
        ...
        {
            $this->colId = $this->createNodeColumn(t("Id"), QQN::OrganizingInstitution()->Id);

            $this->colTitle = $this->createNodeColumn(t("Name"), QQN::OrganizingInstitution()->Name);

            $this->colLockStatus = $this->createNodeColumn(t("Lock status"), QQN::SportsAreas()->IsLockedObject);
            $this->colLockStatus->HtmlEntities = false;

            $this->colStatus = $this->createNodeColumn(t("Is activated"), QQN::OrganizingInstitution()->StatusObject);
            $this->colStatus->HtmlEntities = false;

            $this->colPostDate = $this->createNodeColumn(t("Created"), QQN::OrganizingInstitution()->PostDate);
            $this->colPostDate->Format = 'DD.MM.YYYY hhhh:mm';

            $this->colPostUpdateDate = $this->createNodeColumn(t("Modified"), QQN::OrganizingInstitution()->PostUpdateDate);
        ...
        public function bindData(?QQCondition $objAdditionalCondition = null, mixed $objAdditionalClauses = null): void
        {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = OrganizingInstitution::queryCount($objCondition, $objClauses);
            }
            ...
            $this->DataSource = OrganizingInstitution::queryArray($objCondition, $objClauses);
        }
