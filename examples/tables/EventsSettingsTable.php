<?php
    ...
    /**
     * Class EventsSettingsTable
     *
     * A class responsible for managing the structure and functionality of
     * the EventsSettings table, including its columns, data binding, and
     * query execution. It extends the base VauuTable class, leveraging its
     * features for table rendering and data handling.
     */
    class EventsSettingsTable extends VauuTable
    {
        ...
        public function __construct(mixed $objParent, ?string $strControlId = null)
        {
            ...
            $this->watch(QQN::EventsSettings());
        }

        /**
         * Creates and configures columns for the EventsSettings table.
         *
         * This method initializes various columns with specific configurations such
         * as width, format, and HTML entity handling. The columns are associated
         * with different properties of the EventsSettings object.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
        ...
        {
            $this->colName = $this->createNodeColumn(t("Menu text"), QQN::EventsSettings()->Name);
            $this->colName->CellStyler->Width = '26%';

            $this->colTitle = $this->createNodeColumn(t("Title"), QQN::EventsSettings()->Title);
            $this->colTitle->CellStyler->Width = '26%';

            $this->colIsReserved = $this->createNodeColumn(t("Is reserved"), QQN::EventsSettings()->IsReservedObject);
            $this->colIsReserved->HtmlEntities = false;
            $this->colIsReserved->CellStyler->Width = '12%';

            $this->colStatus = $this->createNodeColumn(t("Status"), QQN::EventsSettings()->StatusObject);
            $this->colStatus->HtmlEntities = false;
            $this->colStatus->CellStyler->Width = '12%';

            $this->colPostDate = $this->createNodeColumn(t("Created"), QQN::EventsSettings()->PostDate);
            $this->colPostDate->Format = 'DD.MM.YYYY hhhh:mm';
            $this->colPostDate->CellStyler->Width = '12%';

            $this->colPostUpdateDate = $this->createNodeColumn(t("Modified"), QQN::EventsSettings()->PostUpdateDate);
        ...
        public function bindData(?QQCondition $objAdditionalCondition = null, mixed $objAdditionalClauses = null): void
        {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = EventsSettings::queryCount($objCondition, $objClauses);
            }
            ...
            $this->DataSource = EventsSettings::queryArray($objCondition, $objClauses);
        }
