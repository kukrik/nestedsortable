<?php
    ...
    /**
     * Represents a table that handles the display and interaction with NewsSettings data.
     * This class provides functionality to create formatted columns, bind data to the table,
     * and manage query conditions and clauses.
     *
     * Properties:
     * - $colName: Represents the column for the "Menu text" field in the table.
     * - $colTitle: Represents the column for the "Title" field in the table.
     * - $colIsReserved: Represents the column for the "Is reserved" field in the table.
     * - $colStatus: Represents the column for the "Status" field in the table.
     * - $colPostDate: Represents the column for the "Created" field, formatted for display.
     * - $colPostUpdateDate: Represents the column for the "Modified" field, formatted for display.
     */
    class NewsSettingsTable extends VauuTable
    {
        ...
        public function __construct(mixed $objParent, ?string $strControlId = null)
        {
            ...
            $this->watch(QQN::NewsSettings());
        }

        /**
         * Creates and configures multiple columns for displaying data.
         *
         * This method initializes various columns used for presenting
         * information such as menu text, titles, status indicators,
         * and timestamps. Each column is styled and configured with
         * specific attributes to ensure appropriate formatting.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
        ...
        {
            $this->colName = $this->createNodeColumn(t("Menu text"), QQN::NewsSettings()->Name);
            $this->colName->CellStyler->Width = '26%';

            $this->colTitle = $this->createNodeColumn(t("Title"), QQN::NewsSettings()->Title);
            $this->colTitle->CellStyler->Width = '26%';

            $this->colIsReserved = $this->createNodeColumn(t("Is reserved"), QQN::NewsSettings()->IsReservedObject);
            $this->colIsReserved->HtmlEntities = false;
            $this->colIsReserved->CellStyler->Width = '12%';

            $this->colStatus = $this->createNodeColumn(t("Status"), QQN::NewsSettings()->StatusObject);
            $this->colStatus->HtmlEntities = false;
            $this->colStatus->CellStyler->Width = '12%';

            $this->colPostDate = $this->createNodeColumn(t("Created"), QQN::NewsSettings()->PostDate);
            $this->colPostDate->Format = 'DD.MM.YYYY hhhh:mm';
            $this->colPostDate->CellStyler->Width = '12%';

            $this->colPostUpdateDate = $this->createNodeColumn(t("Modified"), QQN::NewsSettings()->PostUpdateDate);
        ...
        public function bindData(?QQCondition $objAdditionalCondition = null, mixed $objAdditionalClauses = null): void
        {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = NewsSettings::queryCount($objCondition, $objClauses);
            }
            ...
            $this->DataSource = NewsSettings::queryArray($objCondition, $objClauses);
        }
