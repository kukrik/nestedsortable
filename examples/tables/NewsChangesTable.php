<?php
    ...
    /**
     * Class NewsChangesTable
     *
     * This class extends VauuTable to provide a table-based data representation for the NewsChanges entity.
     * It handles the creation of table columns, data binding with query conditions and clauses,
     * and managing additional conditions and clauses for dynamic configurations.
     */
    class NewsChangesTable extends VauuTable
    {
        ...
        {
            $this->colId = $this->createNodeColumn(t("Id"), QQN::NewsChanges()->Id);
            $this->colTitle = $this->createNodeColumn(t("Title"), QQN::NewsChanges()->Title);
            $this->colStatus = $this->createNodeColumn(t("Is activated"), QQN::NewsChanges()->StatusObject);
            $this->colStatus->HtmlEntities = false;
            $this->colPostDate = $this->createNodeColumn(t("Created"), QQN::NewsChanges()->PostDate);
            $this->colPostDate->Format = 'DD.MM.YYYY hhhh:mm';
            $this->colPostUpdateDate = $this->createNodeColumn(t("Modified"), QQN::NewsChanges()->PostUpdateDate);
        ...
        public function bindData(?QQCondition $objAdditionalCondition = null, mixed $objAdditionalClauses = null): void
        {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = NewsChanges::queryCount($objCondition, $objClauses);
            }
            ...
            $this->DataSource = NewsChanges::queryArray($objCondition, $objClauses);
        }
