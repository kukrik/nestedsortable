<?php
    ...
    /**
     * This class represents a custom table for managing and displaying data related
     * to gallery settings. Extends the base VauuTable functionalities to provide
     * features such as condition management, column creation, data binding, and dynamic property access.
     */
    class GallerySettingsTable extends VauuTable
    {
         ...
         *
         * @param mixed $objParent The parent object to which the control is linked.
        ...
        public function __construct(mixed $objParent, ?string $strControlId = null)
        {
            ...
            $this->watch(QQN::GallerySettings());
        }

        /**
         * Initializes and configures columns for the gallery settings.
         *
         * This method creates and customizes several columns with specific properties,
         * such as width, format, and HTML entity handling, for display and interaction purposes.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
        ...
        {
            $this->colName = $this->createNodeColumn(t("Menu text"), QQN::GallerySettings()->Name);
            $this->colName->CellStyler->Width = '26%';

            $this->colTitle = $this->createNodeColumn(t("Title"), QQN::GallerySettings()->Title);
            $this->colTitle->CellStyler->Width = '26%';

            $this->colIsReserved = $this->createNodeColumn(t("Is reserved"), QQN::GallerySettings()->IsReservedObject);
            $this->colIsReserved->HtmlEntities = false;
            $this->colIsReserved->CellStyler->Width = '12%';

            $this->colStatus = $this->createNodeColumn(t("Status"), QQN::GallerySettings()->StatusObject);
            $this->colStatus->HtmlEntities = false;
            $this->colStatus->CellStyler->Width = '12%';

            $this->colPostDate = $this->createNodeColumn(t("Created"), QQN::GallerySettings()->PostDate);
            $this->colPostDate->Format = 'DD.MM.YYYY hhhh:mm';
            $this->colPostDate->CellStyler->Width = '12%';

            $this->colPostUpdateDate = $this->createNodeColumn(t("Modified"), QQN::GallerySettings()->PostUpdateDate);
        ...
        public function bindData(?QQCondition $objAdditionalCondition = null, mixed $objAdditionalClauses = null): void
        {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = GallerySettings::queryCount($objCondition, $objClauses);
            }
            ...
            $this->DataSource = GallerySettings::queryArray($objCondition, $objClauses);
        }
