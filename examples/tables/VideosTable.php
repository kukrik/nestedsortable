<?php
     ...
     */
    class VideosTable extends VauuTable
    {
         ...
         * Constructor method.
         *
         * @param mixed $objParent The parent object or instance.
         * @param string|null $strControlId Optional control ID for the constructed object.
         *
         * @return void
         * @throws Caller
         */
        public function __construct(mixed $objParent, ?string $strControlId = null)
        {
            ...
            $this->watch(QQN::VideosSettings());
        }

        /**
         * Creates and configures columns for the video settings table.
         * Each column corresponds to a specific attribute of the video settings,
         * including "Videos group", "Title", "Status", "Created", "Modified", and "Author".
         * Additionally, formatting, styling, and ordering configurations are applied to specific columns.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
        ...
        {
            $this->colGroupTitle = $this->createNodeColumn("Videos group", QQN::VideosSettings()->Name);
            $this->colGroupTitle->CellStyler->Width = '15%';

            $this->colTitle = $this->createNodeColumn(t("Title"), QQN::VideosSettings()->Title);
            $this->colTitle->CellStyler->Width = '35%';

            $this->colStatusObject = $this->createNodeColumn(t("Status"), QQN::VideosSettings()->StatusObject);
            $this->colStatusObject->HtmlEntities = false;
            $this->colStatusObject->CellStyler->Width = '12%';

            $this->colPostDate = $this->createNodeColumn(t("Created"), QQN::VideosSettings()->PostDate);
            $this->colPostDate->OrderByClause = QQ::orderBy(QQN::VideosSettings()->PostDate, false);
            $this->colPostDate->ReverseOrderByClause = QQ::orderBy(QQN::VideosSettings()->PostDate, true);
            $this->colPostDate->Format = 'DD.MM.YYYY hhhh:mm';

            $this->colPostUpdateDate = $this->createNodeColumn(t("Modified"), QQN::VideosSettings()->PostUpdateDate);
            $this->colPostUpdateDate->Format = 'DD.MM.YYYY hhhh:mm';

            $this->colAuthor = $this->createNodeColumn(t("Author"), QQN::VideosSettings()->Author);
        }
        ...
        public function bindData(?QQCondition $objAdditionalCondition = null, mixed $objAdditionalClauses = null): void
        {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = VideosSettings::queryCount($objCondition, $objClauses);
            }
            ...
            $this->DataSource = VideosSettings::queryArray($objCondition, $objClauses);
        }
