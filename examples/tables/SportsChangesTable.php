<?php
    ...
    /**
     * Class SportsChangesTable
     *
     * Extends the VauuTable class to provide a table component for managing sports changes.
     * Includes functionalities for dynamic column creation, data binding, and runtime adjustments
     * of specific query conditions and clauses.
     */
    class SportsChangesTable extends VauuTable
    {
        ...
        {
            $this->colId = $this->createNodeColumn(t("Id"), QQN::SportsChanges()->Id);

            $this->colTitle = $this->createNodeColumn(t("Title"), QQN::SportsChanges()->Title);

            $this->colStatus = $this->createNodeColumn(t("Is activated"), QQN::SportsChanges()->StatusObject);
            $this->colStatus->HtmlEntities = false;

            $this->colPostDate = $this->createNodeColumn(t("Created"), QQN::SportsChanges()->PostDate);
            $this->colPostDate->Format = 'DD.MM.YYYY hhhh:mm';

            $this->colPostUpdateDate = $this->createNodeColumn(t("Modified"), QQN::SportsChanges()->PostUpdateDate);
        ...
        public function bindData(?QQCondition $objAdditionalCondition = null, mixed $objAdditionalClauses = null): void
        {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = SportsChanges::queryCount($objCondition, $objClauses);
            }
            ...
            $this->DataSource = SportsChanges::queryArray($objCondition, $objClauses);
        }
