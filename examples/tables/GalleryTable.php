<?php
     ...
     * Class GalleryTable
     *
     * Represents a table for displaying gallery data with support for pagination,
     * sorting, and filtering. Wrapped in a parent VauuTable class, it configures
     * columns for displaying gallery attributes and provides methods to bind and
     * render data with customizable conditions and clauses.
     */
    class GalleryTable extends VauuTable
    {
        ...
        public function __construct(mixed $objParent, ?string $strControlId = null)
        {
            ...
            $this->watch(QQN::GalleryList());
        }

        /**
         * Creates and configures the columns for the gallery list, including their titles, formats, widths,
         * and other specific properties such as sorting and formatting options.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
        ...
        {
            $this->colGroupTitle = $this->createNodeColumn("Gallery group", QQN::GalleryList()->GroupTitle);
            $this->colGroupTitle->CellStyler->Width = '10%';

            $this->colTitle = $this->createNodeColumn(t("Title"), QQN::GalleryList()->Title);
            $this->colTitle->CellStyler->Width = '40%';

            $this->colStatusObject = $this->createNodeColumn(t("Status"), QQN::GalleryList()->StatusObject);
            $this->colStatusObject->HtmlEntities = false;
            $this->colStatusObject->CellStyler->Width = '10%';

            $this->colPostDate = $this->createNodeColumn(t("Created"), QQN::GalleryList()->PostDate);
            $this->colPostDate->OrderByClause = QQ::orderBy(QQN::GalleryList()->PostDate, false);
            $this->colPostDate->ReverseOrderByClause = QQ::orderBy(QQN::GalleryList()->PostDate, true);
            $this->colPostDate->Format = 'DD.MM.YYYY hhhh:mm';

            $this->colPostUpdateDate = $this->createNodeColumn(t("Modified"), QQN::GalleryList()->PostUpdateDate);
            $this->colPostUpdateDate->Format = 'DD.MM.YYYY hhhh:mm';
            $this->colAuthor = $this->createNodeColumn(t("Album author"), QQN::GalleryList()->Author);
        }
        ...
        public function bindData(?QQCondition $objAdditionalCondition = null, mixed $objAdditionalClauses = null): void
        {
            ...
            if ($this->Paginator) {
                $this->TotalItemCount = GalleryList::queryCount($objCondition, $objClauses);
            }
            ...
            $this->DataSource = GalleryList::queryArray($objCondition, $objClauses);
        }
