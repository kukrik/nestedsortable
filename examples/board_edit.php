<?php
    ...
    /**
     * This class represents the BoardEditForm, which is responsible for managing and editing board properties and
     * related inputs. It extends the base Form class, providing additional functionality and UI components for board
     * management.
     *
     * The class includes various protected properties for modal dialogs, toastr notifications, labels, text inputs,
     * dropdown lists, date controls, etc., required for maintaining board-related data. It also incorporates
     * mechanisms
     * for resetting input visibility, creating inputs and controls dynamically, sorting inputs, and managing modal
     * dialogs.
     *
     * Noteworthy properties include:
     *  - dlgModalX: Modal dialogs (e.g., dlgModal1, dlgModal2, etc.) for various interactive operations.
     *  - dlgToastrX: Notifications using toastr for user feedback.
     *  - lblInfo, lblGroupTitle, lblTitleSlug: Informational labels.
     *  - txtNewFullName, txtFullName, txtPosition, etc.: Text inputs for user data entry.
     *  - lstBoardStatus, lstStatus: Dropdowns for status selection.
     *  - calPostDate, calPostUpdateDate: Calendar controls for date and time inputs.
     *  - Buttons such as btnAddMember, btnMemberSave, btnUpdate, etc., for form actions.
     *  - Other variables related to user sessions, board settings, and media finder integration.
     *
     * The initialization process is carried out through `formCreate`, where inputs, buttons, and other components are
     * set up. Dynamic and customizable functionalities are handled through various helper methods such as
     * resettingInputs and createInputs, making the form flexible and user-friendly.
     */
    class BoardEditForm extends Form
    {
        ...
        protected Bs\TextBox $txtNewFullName;
        ...
        protected Bs\TextBox $txtFullName;
        protected Q\Plugin\Control\Label $lblFullName;
        protected Bs\TextBox $txtPosition;
        protected Q\Plugin\Control\Label $lblPosition;
        protected Bs\TextBox $txtAreasResponsibility;
        protected Q\Plugin\Control\Label $lblAreasResponsibility;
        protected Bs\TextBox $txtInterests;
        protected Q\Plugin\Control\Label $lblInterests;
        ...
        protected Q\Plugin\RadioList $lstBoardStatus;
        protected Q\Plugin\Control\Label $lblBoardStatus;
        ...
        protected ?object $objBoard = null;
        ...
        /**
         * Initializes the form and its components during the creation phase.
         *
         * @return void
         * @throws Caller
         * @throws InvalidCast
        ...
        protected function formCreate(): void
        {
            ...
            //Application::executeJavaScript("$('.board-image-wrapper').addClass('hidden');");
        ...
        protected function createInputs(): void
        {
            ...
            $this->txtNewFullName->CrossScripting = Q\Control\TextBoxBase::XSS_HTML_PURIFIER;
         ...
         * @throws InvalidCast
         */
        public function Sorter_Draw(Board $objBoard): array
        {
         ...
         * @throws Caller
         */
        public function Buttons_Draw(Board $objBoard): string
        {
        ...
        protected function createInputManager(): void
        {
            ...
            $this->txtFullName->AddAction(new EnterKey(), new Ajax('btnUpdate_Click'));
            $this->txtFullName->addAction(new EnterKey(), new Terminate());
            $this->txtFullName->AddAction(new EscapeKey(), new Ajax('itemEscape_Click'));
            $this->txtFullName->addAction(new EscapeKey(), new Terminate());
            ...
            $this->txtPosition->AddAction(new EnterKey(), new Ajax('btnUpdate_Click'));
            $this->txtPosition->addAction(new EnterKey(), new Terminate());
            $this->txtPosition->AddAction(new EscapeKey(), new Ajax('itemEscape_Click'));
            $this->txtPosition->addAction(new EscapeKey(), new Terminate());
            ...
            $this->txtAreasResponsibility->AddAction(new EnterKey(), new Ajax('btnUpdate_Click'));
            $this->txtAreasResponsibility->addAction(new EnterKey(), new Terminate());
            $this->txtAreasResponsibility->AddAction(new EscapeKey(), new Ajax('itemEscape_Click'));
            $this->txtAreasResponsibility->addAction(new EscapeKey(), new Terminate());
            ...
            $this->txtInterests->AddAction(new EnterKey(), new Ajax('btnUpdate_Click'));
            $this->txtInterests->addAction(new EnterKey(), new Terminate());
            $this->txtInterests->AddAction(new EscapeKey(), new Ajax('itemEscape_Click'));
            $this->txtInterests->addAction(new EscapeKey(), new Terminate());
            ...
            $this->lstBoardStatus->addAction(new Change(), new Ajax('lstBoardStatus_Change'));
            $this->lstBoardStatus->AddAction(new EnterKey(), new Ajax('btnUpdate_Click'));
            $this->lstBoardStatus->addAction(new EnterKey(), new Terminate());
            $this->lstBoardStatus->AddAction(new EscapeKey(), new Ajax('itemEscape_Click'));
            $this->lstBoardStatus->addAction(new EscapeKey(), new Terminate());
        ...
        /**
         * Initializes multiple modal dialogs with various configurations, including text, title, header classes,
         * buttons, and actions. These modals provide feedback and confirmation requests for different user actions
         * related to board groups and menu items.
         *
         * @return void
         */
        protected function createModals(): void
        {
            ...
            $this->dlgModal5->Text = t('<p style="line-height: 25px; margin-bottom: 2px;">Are you sure you want to permanently delete this board member?</p>
            ...
            $this->dlgModal8->Text = t('<p style="line-height: 25px; margin-bottom: 2px;"><strong>Note:</strong> Please add at least one member to the board group before making it public.</p>
            ...
            $this->dlgModal9->Text = t('<p style="line-height: 25px; margin-bottom: 2px;">Board members have been selected, but all members are hidden, so there is no reason to make the board group public. </p>
                                    <p style="line-height: 25px; margin-bottom: -3px;">The board group is now hidden.</p>');
            ...
            $this->dlgModal10->Text = t('<p style="line-height: 25px; margin-bottom: 2px;">Board members have been selected, but all members are hidden, so there is no reason to make the board group public. </p>');
        ...
        {
            $objBoardsSettings = BoardsSettings::loadById($this->intId);
            ...
            if ($objBoardsSettings->getAllowedUploading()) {
                Application::executeJavaScript("
                   $('.board-setting-wrapper').addClass('hidden');
                   $('.board-image-wrapper').addClass('hidden');
                   $('.form-actions-wrapper').addClass('hidden');
        ...
        /**
         * Handles the change event for the board status list.
         *
         * @param ActionParams $params Parameters associated with the action triggering the change.
         *
         * @return void
         ...
         * @throws InvalidCast
         */
        protected function lstBoardStatus_Change(ActionParams $params): void
        {
            $objBoard = Board::loadById($this->intClick);
            $objMenuContent = MenuContent::loadById($this->objMenu->getId());

            $objBoards = Board::loadArrayByBoardId($this->intId);
            $enabledStatus = 0;

            if ($this->lstBoardStatus->SelectedValue == 2) {
...
//            }
        ...
        /**
         * Handles the change event for the status dropdown list. This method updates the status of the menu content
         * and displays appropriate dialog boxes based on various conditions, such as the presence of boards,
         * the selected status, and menu hierarchy.
         *
...
//            }
            ...
            if ($this->objBoardsSettings->getStatus() === 2) {
        ...
        /**
         * Updates the status of board settings by recording the current date, assigning the editor's name,
        ...
        protected function deleteItem_Click(ActionParams $params): void
        {
            ...
            if ($this->objBoardsSettings->getAllowedUploading()) {
                Application::executeJavaScript("
                   $('.board-setting-wrapper').addClass('hidden');
                   $('.board-image-wrapper').addClass('hidden');
                   $('.form-actions-wrapper').addClass('hidden');
    ...
    BoardEditForm::run('BoardEditForm');